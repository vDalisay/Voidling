...
public partial class SettingsScreen : VBoxContainer
{
    ...
    public override void _Ready()
    {
        ...
        AddChild(BuildVolumeRow("Master", _state.MasterVolume, value => { _state = _state with { MasterVolume = value }; MasterVolumeChanged?.Invoke(value); }));
        AddChild(BuildVolumeRow("SFX", _state.SoundEffectVolume, value => { _state = _state with { SoundEffectVolume = value }; SoundEffectVolumeChanged?.Invoke(value); }));
        AddChild(BuildVolumeRow("UI", _state.UiSoundVolume, value => { _state = _state with { UiSoundVolume = value }; UiSoundVolumeChanged?.Invoke(value); }));
    ...
    private Button BuildEdgePanButton()
    {
        ...
        button.Pressed += () => { _state = _state with { EdgePanning = button.ButtonPressed }; button.Text = EdgePanText(button.ButtonPressed); EdgePanningChanged?.Invoke(button.ButtonPressed); };
        return button;
    }
    ...
    private Button BuildAutoFinishButton()
    {
        ...
        button.Pressed += () => { _state = _state with { AutoFinishRaces = button.ButtonPressed }; button.Text = AutoFinishText(button.ButtonPressed); AutoFinishRacesChanged?.Invoke(button.ButtonPressed); };
        return button;
    }

    private string FormatVolume(float percent) => string.Format(Tr("UI_SETTINGS_VOLUME"), Mathf.RoundToInt(percent));
    private string EdgePanText(bool enabled) => Tr(enabled ? "UI_SETTINGS_EDGE_PAN_ON" : "UI_SETTINGS_EDGE_PAN_OFF");
    private string AutoFinishText(bool enabled) => Tr(enabled ? "UI_SETTINGS_AUTO_FINISH_ON" : "UI_SETTINGS_AUTO_FINISH_OFF");
}
