...
public sealed class GodotAudioSettingsAdapter : IAudioSettingsAdapter
{
    ...
    public GodotAudioSettingsAdapter(string masterBusName = "Master", string soundEffectBusName = "SFX", string uiSoundBusName = "UI")
    {
    ...
    public void ApplyMasterVolume(float linearVolume) => ApplyBusVolume(_masterBusName, linearVolume);
    public void ApplySoundEffectVolume(float linearVolume) => ApplyBusVolume(_soundEffectBusName, linearVolume);
    public void ApplyUiSoundVolume(float linearVolume) => ApplyBusVolume(_uiSoundBusName, linearVolume);
    ...
    private static void ApplyBusVolume(string busName, float linearVolume)
    {
        ...
        if (volume > 0.001f) AudioServer.SetBusVolumeDb(bus, Mathf.LinearToDb(volume));
    }
}
