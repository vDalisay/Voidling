...
public sealed class ConnectedZoneServiceTests
{
    ...
    public void HostPublish_AppliesCanonicalStateAndBroadcastsReliableEvent()
    {
        ...
        Assert.True(MultiplayerProtocol.TryDecodeVoidlingPublishedEvent(
            sent.Payload.Span,
            host.Id,
            out _,
            out var epoch,
    ...
    public void ClientConstruction_RequestsFullSnapshotFromCurrentHost()
    {
        ...
        Assert.Equal(NetworkChannel.Zone, sent.Channel);
        Assert.True(MultiplayerProtocol.TryDecodeZoneSnapshotRequest(
            sent.Payload.Span,
            client.Id,
            out _,
            out var lobbyId));
    ...
    public void HostRejectsPublishThatClaimsAnotherOwner()
    {
        ...
        transport.Emit(new NetworkPacket(
            remote.Id,
            NetworkChannel.Zone,
            MultiplayerProtocol.EncodePublishVoidlingCommand(remote, forged)));
    ...
    public void ClientAppliesFullSnapshotOnlyFromLobbyHost()
    {
        ...
        transport.Emit(new NetworkPacket(
            host.Id,
            NetworkChannel.Zone,
            MultiplayerProtocol.EncodeZoneSnapshot(host, full)));
    ...
    public void SteamOwnerMigration_PreservesReplicatedStateAndIncrementsEpoch()
    {
        ...
        transport.Emit(new NetworkPacket(
            oldHost.Id,
            NetworkChannel.Zone,
            MultiplayerProtocol.EncodeZoneSnapshot(oldHost, full)));
        transport.Sent.Clear();
        ...
        Assert.Contains(transport.Sent, message =>
            message.Peer == oldHost.Id && message.Channel == NetworkChannel.Zone);
    }

    private static LobbySnapshot CreateLobby(
        PlatformUser owner,
        params PlatformUser[] members)
        => new(
            77,
            owner.Id,
            members.Select(user => new LobbyMember(user, user.Id == owner.Id)).ToArray());

    private static SharedVoidlingSnapshot CreateSharedVoidling(
