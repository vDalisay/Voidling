...
Wings, crystals and similar combinatorial parts are separate layer sprites registered on the owning body definition.
...
- `LayerId` — stable variant ID such as `wings/basic`;
- `SlotId` — replacement group such as `wings` or `crystal`;
...
The `normal` definition still points at the existing Sprout Lands character/swim atlases until the production sprites arrive. It intentionally declares no source palette slots, so the renderer keeps the legacy flat-tint fallback for that art. Production definitions switch automatically to palette-slot recoloring when their source palettes are authored.
